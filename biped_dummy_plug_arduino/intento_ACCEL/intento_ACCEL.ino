...
void setup(){
  ...
  pinMode(pinAccelZ, INPUT);          //interruptor de AccelZ
  Serial.begin(9600);
...
float datos(int pin) {
  for (int i=0 ; i<10;i++){
    DatosAccelZ[i]=digitalRead(pin);
    i+=1;
    delay(3);
    //PromedioAccelZ=mean(DatosAccelZ);
    
    }
  float PromedioAccelZ=mean(DatosAccelZ,10);
  return PromedioAccelZ;
  ...
  while (millis() - tinicial < tiempo) {
    ...
    if (estadoAccelZ==0){
      ...
      for(int j=0;j<LARGOLISTA;j+=1){
         DatosAccelZ[j]=estadoAccelZ;
         estadoAccelZ=analogRead(A4);
         delay(10);
      }
      for(int j=0;j<LARGOLISTA;j+=1){
         SumAccelZ+=DatosAccelZ[j];
      }
      if (SumAccelZ<15){//estadoAccelZ == 0) { 
...
int tomacaida9nueva;
int tomacaida9vieja;
int tomacaida10nueva;
int tomacaida10vieja;
