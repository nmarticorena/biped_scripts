...
void update (int tiempo) {   //REEMPLAZA A TODOS LOS DELAY
  ...
  while (millis() - tinicial < tiempo) {
    ...
    if (estadoMercurio==0){
      ...
      if (SumMercurio<5){//estadoMercurio == 0) { 
      ...
      while (true){
     // estadoMercurio = digitalRead(13);
     estadoMercurio = false;
      Serial.println("MERCURIOOOOOOOOOOOOOOOOOOOOOOOOO");
    
      if (estadoMercurio == true) { 
          Serial.println("YA NO MERCURIOOOOOOOOOOOOOOOOOOOOOOOOO");
          delay(4000);
          leftHipServo.attach(11);
          leftKneeServo.attach(10);
          leftAnkleServo.attach(9);
